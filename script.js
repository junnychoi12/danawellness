...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(bookForm){
    bookForm.addEventListener('submit', (e) => {
      ...
      const subject = encodeURIComponent(`Appointment Request — ${checked[0] || 'danA Wellness Clinic'}`);
      const body = encodeURIComponent(
        `Name: ${name}\nEmail: ${email}\nPhone: ${phone}\nRequested Treatment(s): ${treatment}\nPreferred Date: ${date}\nPreferred Time: ${time}\nNotes: ${notes}`
      );
      window.location.href = `mailto:info@danawellnessclinic.com?subject=${subject}&body=${body}`;
  ...
  if(contactForm){
    contactForm.addEventListener('submit', (e) => {
      ...
      const subject = encodeURIComponent(`Message from ${name} — danA Wellness Clinic Website`);
